...
test('SettingsStore persists safe startup connection staggering', (t) => {
  ...
  assert.deepEqual(store.get(), { staggerStartupConnections: true, startupConnectionDelay: 3, uiScale: 100, sidePanelWidth: 300, inventoryHeight: 260, macros: [] })
  ...
  assert.deepEqual(store.get(), { staggerStartupConnections: false, startupConnectionDelay: 12, uiScale: 100, sidePanelWidth: 300, inventoryHeight: 260, macros: [] })
  assert.deepEqual(normalizeSettings({ startupConnectionDelay: 9999 }), { staggerStartupConnections: true, startupConnectionDelay: 300, uiScale: 100, sidePanelWidth: 300, inventoryHeight: 260, macros: [] })
...
test('SettingsStore bounds persistent internal panel sizes', () => {
  ...
  assert.deepEqual({ sidePanelWidth: largeInventory.sidePanelWidth, inventoryHeight: largeInventory.inventoryHeight }, { sidePanelWidth: 240, inventoryHeight: 420 })
})
